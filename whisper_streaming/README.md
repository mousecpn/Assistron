...
### Push-to-talk client and request-response server

If you want a request-response workflow (hold key to record, release to send, receive one transcript), use:

- `whisper_http_server.py`: one-shot HTTP transcription endpoint (`POST /transcribe` with wav body).
- `push_to_talk_client.py`: desktop client that records while SPACE is held, then sends audio on release and prints transcript.
...
```
python3 whisper_http_server.py --host 0.0.0.0 --port 43100 --lan en --model large-v2 --backend faster-whisper
```

Start client:

```
python3 push_to_talk_client.py --server http://127.0.0.1:43100
```

Client controls:

- Hold SPACE: start recording.
- Release SPACE: stop recording and send request.
- Press ESC: exit client.
